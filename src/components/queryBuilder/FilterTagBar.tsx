...
const getFilterDisplayValue = (filter: Filter): string => {
  if (filter.operator === FilterOperator.IsAnything) return '';
  if (filter.operator === FilterOperator.IsNull || filter.operator === FilterOperator.IsNotNull) return '';
  if (filter.operator === FilterOperator.IsEmpty || filter.operator === FilterOperator.IsNotEmpty) return '';
  if (filter.operator === FilterOperator.WithInGrafanaTimeRange) return 'dashboard time';
  if (filter.operator === FilterOperator.OutsideGrafanaTimeRange) return 'outside dashboard time';
  if ('value' in filter && filter.value !== undefined) {
    const v = String(filter.value);
    return v.length > 30 ? v.substring(0, 27) + '...' : v;
  }
  return '';
};

const getOperatorDisplay = (op: FilterOperator): string => {
  switch (op) {
    case FilterOperator.IsAnything: return 'is anything';
